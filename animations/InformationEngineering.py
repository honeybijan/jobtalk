...
class InformationEngineering(PresentationScene):
    ...
    def construct(self):
        ...
        AX = Edge(A, X, observed=False, redraw=False)
        AY = Edge(A, Y, observed=False, redraw=False)
        BY = Edge(B, Y, observed=False, redraw=False)
        causal_dag = VGroup(X, Y, A, B, AX, AY, BY)
        causal_dag.move_to(ORIGIN).scale(2)
        self.play(Write(causal_dag), run_time = 1)
        self.end_fragment()

        PartialBlue = VGroup(X, Y, A, AX, AY)
