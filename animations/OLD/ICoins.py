...
class ICoins(PresentationScene):
    ...
    def construct(self):
        ...
        Coins_and_Data = VGroup(Data_Group, all_coins, Data_Label, Data_Label_Line, U_values)
        ...
        Vertices = [Vertex(label = r'V').next_to(coin_list[0][i], UP).shift(.5 * UP) for i in range(7)]
        Mixture_U = Vertex(label=r"U", observed=False).next_to(Vertices[3], UP).shift(.75 * UP)
        Edges = [Edge(Mixture_U, V, observed=False) for V in Vertices]
